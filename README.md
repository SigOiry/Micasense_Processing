...
**WORK IN PROGRESS (last update : 2024-02-07 16:01:47.610404)**
...
corners as shown on this image. Note that the Y-axis references used by
`opencv` and `terra` differ, necessitating a correction to ensure that
points are plotted correctly on the image.

<details>
<summary>Code</summary>

``` r
img<-opencv::ocv_read('Dual_MX_Images/Red/IMG_0002_1.tif') ### Open the image with opencv

img_rast<-rast('Dual_MX_Images/Red/IMG_0002_1.tif') ### open the image with Terra 
names(img_rast)<- "value"

y_dim<-exif_read('Dual_MX_Images/Red/IMG_0002_1.tif')$ImageHeight ### Retrieve the number of rows of the image

qr_coordinate<-attr(opencv::ocv_qr_detect(img),which = "points") %>% 
  as.data.frame() %>% 
  rename(x = "V1",
         y = "V2") %>% 
...
plot_qr<-ggplot()+
  ...
  geom_point(data = qr_coordinate, aes(x =x , y = y), color = "red", size = 3)+
  geom_text(data = qr_coordinate, aes(x =x+30 , y = y+30, label = names), fontface = "bold",color = "red", size = 6)+
  theme_void()+
