...
To correct the original image, we simply need to multiply the image by the vignetting map.
...
names(img_corrected)<-"value"

RAW_plot<-ggplot()+
  geom_spatraster(data = img_raw, aes(fill = value))+
  labs(fill = "DN")+
...
Corrected_plot <- ggplot()+
  geom_spatraster(data = img_corrected, aes(fill = value))+
  labs(fill = "DN")+
  scale_fill_gradientn(
    colours = grey(0:100 / 100),
    na.value = "transparent",
