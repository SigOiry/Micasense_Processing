...
def focal_plane_resolution_px_per_mm(meta):
  fp_x_resolution = float(meta.get('FocalPlaneXResolution')[0])
  fp_y_resolution = float(meta.get('FocalPlaneYResolution')[0])
  
  return fp_x_resolution, fp_y_resolution

def principal_point_px(meta):
  
    np.float_(meta.get("PrincipalPoint")[0].replace(","," ").split())
    center_x = np.float_(meta.get("PrincipalPoint")[0].replace(","," ").split())[0] * focal_plane_resolution_px_per_mm(meta)[0]
    center_y = np.float_(meta.get("PrincipalPoint")[0].replace(","," ").split())[1] * focal_plane_resolution_px_per_mm(meta)[1]
...
def cv2_camera_matrix(meta):
  ...
  cam_mat[0, 0] = meta.get("FocalLength")[0] * focal_plane_resolution_px_per_mm(meta)[0]
  cam_mat[1, 1] = meta.get("FocalLength")[0] * focal_plane_resolution_px_per_mm(meta)[1]
  cam_mat[2, 2] = 1.0
...
def cv2_distortion_coeff(meta):
  ...
  nelem = np.array(meta.get('PerspectiveDistortion')[0]).size
...
def get_homography(match, ref, R = None, T = None):
  R = rotations_degrees_to_rotation_matrix(np.float_(match.get("RigRelatives")[0].replace(',', '').split()))
  
  if T is None:
    T = np.zeros(3)
    
  R_ref = rotations_degrees_to_rotation_matrix(np.float_(ref.get("RigRelatives")[0].replace(',', '').split()))
...
def rig_xy_offset_in_px(meta):
  ...
  px_fov_x = 2.0 * math.atan2(pixel_pitch_mm_x / 2.0, meta.get("FocalLength")[0])
  px_fov_y = 2.0 * math.atan2(pixel_pitch_mm_y / 2.0, meta.get("FocalLength")[0])
  t_x = math.radians(float(meta.get("RigRelatives")[0].replace(",","").split()[0])) / px_fov_x
  t_y = math.radians(float(meta.get("RigRelatives")[0].replace(",","").split()[1])) / px_fov_y
  return t_x, t_y
...
def align(pair,wrap_matrix):
  ...
  max_iterations = int(pair['max_iteration'][0])
  epsilon_threshold = pair['epsilon_threshold'][0]
  ref_index = pair['ref_index'][0]
  match_index = pair['match_index'][0]
  translations = np.float_(pair['translation'][0].split())
  wrap_matrix_match = wrap_matrix.copy()
  ref = cv2.imread(pair['ref_path'][0],cv2.IMREAD_UNCHANGED)
  # ref = rawpy.imread('Dual_MX_Images/Blue/IMG_0002_10.tif')
  w = ref.shape[1]
  ...
  if ref_index != match_index:
    ...
    gray1 = cv2.imread(pair["ref_path"][0],cv2.IMREAD_UNCHANGED)
    gray2 = cv2.imread(pair["match_path"][0],cv2.IMREAD_UNCHANGED)
  ...
  img = cv2.imread(pair["match_path"][0],cv2.IMREAD_UNCHANGED)
  
  im_aligned = np.zeros((height, width), dtype=np.float32)

  im_aligned = cv2.warpPerspective(img,warp_matrix,(height,width), flags=interpolation_mode + cv2.WARP_INVERSE_MAP)
...
for( i in 1:length(unique(output$CaptureId))){
  ...
  for(ii in 1:nrow(capture)){
    ...
    alignment_pairs = data.frame(max_iteration = 2500,
      ...
      mutate(output_path = paste0("Output/aligned/", gsub(".*/","",match_path)))
    
    py$align(alignment_pairs)
...
def align(pairs): 
  ...
  ref = pairs["ref_path"][0]  
  match = pairs["match_path"][0]
  output = pairs["output_path"][0]

  img_ref = cv2.imread(ref,cv2.IMREAD_UNCHANGED)
  img_match = cv2.imread(match,cv2.IMREAD_UNCHANGED)
  ...
  # im2_aligned = cv2.warpPerspective (img_match, warp_matrix, (1290, 960), flags=cv2.INTER_LINEAR + cv2.WARP_INVERSE_MAP) ## HOMOGRAPHY
  
  im2_aligned = cv2.warpAffine(img_match, warp_matrix, (1290, 960), flags=cv2.INTER_LINEAR + cv2.WARP_INVERSE_MAP);
